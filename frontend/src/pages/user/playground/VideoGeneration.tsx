import { AlertCircle, Loader2, Lock,Play, Square, Upload, XCircle } from 'lucide-react';
...
import { ModelOption,taskService, TaskStatusResponse } from '@/services/tasks';
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        ...
        const data = await taskService.getAvailableModels();
        setModels(data);
        
        // 默认选中第一个可用模型
        const firstAvailable = data.find(m => m.is_available);
        if (firstAvailable) {
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button 
                variant={duration === '5s' ? 'default' : 'outline'} 
              ...
              <Button 
                variant={duration === '10s' ? 'default' : 'outline'} 
