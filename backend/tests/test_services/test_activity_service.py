...
class TestClaimActivity:
    ...
    def test_claim_activity_not_started(self, db_session, test_user):
        ...
        activity = Activity(
            ...
            name='未来的活动',
            points=50.00,
            start_at=datetime.now(ZoneInfo("Asia/Shanghai")) + timedelta(days=1),
            status='active'
        )
        db_session.session.add(activity)
        db_session.session.commit()
    ...
    def test_claim_activity_ended(self, db_session, test_user):
        ...
        activity = Activity(
            ...
            name='过去的活动',
            points=50.00,
            end_at=datetime.now(ZoneInfo("Asia/Shanghai")) - timedelta(days=1),
            status='active'
        )
        db_session.session.add(activity)
        db_session.session.commit()
...
class TestDailyCheckin:
    ...
    def test_consecutive_checkin(self, db_session, test_user):
        ...
        # 第一天签到
        user.last_checkin_at = datetime.now(ZoneInfo("Asia/Shanghai")) - timedelta(days=1)
    ...
    def test_broken_checkin_streak(self, db_session, test_user):
        ...
        # 模拟3天前签到过
        user.last_checkin_at = datetime.now(ZoneInfo("Asia/Shanghai")) - timedelta(days=3)
...
class TestExpireActivityPoints:
    ...
    def test_expire_single_claim(self, db_session, test_user, test_activity):
        ...
        claim.expire_at = datetime.now(ZoneInfo("Asia/Shanghai")) - timedelta(days=1)
        db_session.session.commit()
