...
**说明**: 登出时会清除 Redis 中的 token，实现单点登出。登出后该 token 将失效，需要重新登录获取新 token。
...
5. 用户登出时调用登出接口清除服务端 token：
   - POST `/api/auth/logout`，清除 Redis 中的 token 记录
6. 如果后端返回 401（Invalid token），提示用户重新登录并获取新 token。
