...
- 批次号用于后续批量作废操作
 - 新增可选字段 `grant_level`（整数 1-5），用于在用户兑换该 CDK 时提升用户会员等级：
   * 如果 CDK 包含 `grant_level`，兑换成功后系统会判断用户当前等级是否低于 `grant_level`，若低于则把用户等级提升到 `grant_level`，并在流水备注中记录升级信息。
   * 管理端在生成 CDK 时可填写 `grant_level`，前端列表会显示该字段以供管理员参考。
